...
@testset "Custom: BPE - new API" begin
    txt = tempname()*".txt"; write(txt, "low lower lowest\nslow shower show")

    model = train_bpe([txt]; vocab_size=256, num_merges=400, model_path="toy.bpe")
    enc   = load_bpe(model)

    ids   = encode(enc, "lowest shower")
    @assert decode(enc, ids) == "lowest shower"
end


@testset "Custom: Basic Custom BPE functionality" begin
    # Create a temporary text file with simple content
    txt = tempname() * ".txt"
    write(txt, "low lower lowest\nslow shower show")
    
    # Train the BPE model
    model_path = train_bpe([txt]; vocab_size=256, num_merges=400, model_path="toy1.bpe")
    enc = load_bpe(model_path)
    
    # Test round-trip encoding/decoding with words from training data
    test_str = "lowest shower"
    ids = encode(enc, test_str)
    decoded = decode(enc, ids)
    
    println("Original: \"$test_str\"")
    println("Encoded IDs: $ids")
    println("Decoded: \"$decoded\"")
    
    @test decoded == test_str
...
#     ids, enc = PP.preprocess_for_subword_embeddings(txt;
#                    bpe_vocab_size = 256, bpe_merges = 1_000)

#     @test !isempty(ids) && isa(enc, BytePairEncoding.BPEEncoder)
#     model, _ = SWE.train!(ids; enc = enc, epochs = 1, emb_dim = 32)
#     @test size(SWE.embeddings(model)) == (32, SWE.used_vocab_size(enc))
# end
