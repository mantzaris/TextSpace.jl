...
@testset "auto-BPE preprocessing helper" begin
    txt = "Low lower lowest. Wider newer. Slow snow shower."
    sentences, enc = PP.preprocess_for_subword_embeddings(txt;
                      bpe_vocab_size = 256, bpe_merges = 1_000)

    @test !isempty(sentences) && isa(enc, BytePairEncoding.BPEEncoder)

    model, _ = SWE.train_custom!(sentences; enc = enc, epochs = 1, emb_dim = 32)

    @test size(SWE.embeddings(model)) == (32, SWU.full_vocab_size(enc))

    @test SWU.full_vocab_size(enc) == length(enc.vocab.list)

    # every defined id has a column
    E = SWE.embeddings(model)
    for (tok,id) in SWE.each_token(enc)
        @test size(E,2) ≥ id       # never hits column 0
    end
end

@testset "auto-BPE – extended checks" begin
    txt  = "Low lower lowest. Wider newer. Slow snow shower."
    sents, enc = PP.preprocess_for_subword_embeddings(txt;
                      bpe_vocab_size = 256, bpe_merges = 1_000)

    @test !isempty(sents) && isa(enc, BytePairEncoding.BPEEncoder)

    # 1. CBOW
    m_cbow, _ = SWE.train_custom!(sents; enc=enc,
                                  objective=:cbow,
                                  epochs=1, emb_dim=24)
    @test size(SWE.embeddings(m_cbow)) == (24, SWU.full_vocab_size(enc))

    # 2. round-trip (allow inner blanks)
    sent  = "low lower"
    @test occursin(r"low\s+lower",
                   decode(enc, encode(enc,sent; add_special_tokens=true)))

    # 3. batch padding
    A,B   = "low slow","shower slow snow"
    mat   = PP.encode_batch(enc, [A,B]; add_special_tokens=true)
    @test mat[end,1] == PP.special_id(enc,"<pad>")

    # 4. UNK present (not necessarily the only id)
    oov_ids  = encode(enc, "THISWORDDOESNOTEXIST")
    @test !isempty(oov_ids) && !isempty(decode(enc, oov_ids))

    # 5. merge sanity – ≤4 sub-tokens is fine for a 1 k-merge model
    tok = enc.tokenizer
    @test length(tok("lowest"))  ≤ 4
    @test length(tok("shower"))  ≤ 4
end
